...
export default function MissionScreen() {
  ...
  const [missionMoodText,  setMissionMoodText]  = useState('');
  ...
  const handleConfirm = () => {
    ...
    if (newDone >= TOTAL_MISSIONS) {
      ...
      setMissionMoodText('');
      setShowMissionMood(true);
    }
  };

  // ── 미션 완수 후 소감 저장 ────────────────────────────────────────────────────
  const handleMissionMoodSave = () => {
    const text = missionMoodText.trim();
    if (text && currentUsername) {
      import('../lib/supabase').then(({ saveMoodLog, isSupabaseConfigured }) => {
        if (isSupabaseConfigured()) {
          saveMoodLog({
            user_name:  currentUsername,
            mood_id:    'post_mission_text',
            mood_label: '활동 후 소감',
            memo:       text,
            logged_at:  new Date(Date.now() + 9 * 3600 * 1000).toISOString(),
      ...
      {showMissionMood && (
        ...
        >
          <View style={s.celebCard}>
            ...
            <Text style={[s.celebSub, { marginBottom: 8 }]}>
              {'활동 후 기분이 어떤가요?\n솔직하게 적어봐요 💙'}
            </Text>
            <TextInput
              style={[s.textInput, { width: '100%', minHeight: 80, textAlignVertical: 'top' }]}
              value={missionMoodText}
              onChangeText={t => setMissionMoodText(t.slice(0, 100))}
              placeholder="몸을 움직이니까 기분이 괜찮아졌어요."
              placeholderTextColor={COLORS.border}
              multiline
              maxLength={100}
              autoFocus
            />
            <Text style={[s.charCount, { alignSelf: 'flex-end' }]}>{missionMoodText.length} / 100</Text>
            <TouchableOpacity
              style={[s.confirmBtn, { width: '100%', marginTop: 4 }, !missionMoodText.trim() && { opacity: 0.4 }]}
              onPress={() => { if (missionMoodText.trim()) handleMissionMoodSave(); }}
              disabled={!missionMoodText.trim()}
              activeOpacity={0.8}
            >
